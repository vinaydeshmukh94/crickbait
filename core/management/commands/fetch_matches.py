...
class Command(BaseCommand):
    ...
    def update_bets_table(self, match_info, match):
        # Check if match has completed
        winning_point = 1
        losing_point = 0
        if match_info['state'].lower() == "complete":  # Or however 'completed' is represented
            print("match status complete")
            ...
            print(match.id, Bet.objects.filter(match=match, chosen_team=winning_team_id).count())
            if winning_team_id and losing_team_id:
                winning_update_count = Bet.objects.filter(match=match, chosen_team=winning_team_id).update(
                    is_correct=True,
                    points_awarded=winning_point
                )
                losing_update_count = Bet.objects.filter(match=match, chosen_team=losing_team_id).update(
                    is_correct=False,
                    points_awarded=losing_point
                )
                logging.info(f"match-{match.id}, winning-update-{winning_update_count}, losing-update-{losing_update_count}")
    ...
    def handle(self, *args, **kwargs):
        ...
        for match_data in data.get('matchDetails', []):
            ...
            for match_info in match_data.get('matchDetailsMap', {}).get('match', []):
                ...
                self.update_bets_table(match_info=m, match=match)
                count += 1
